...
class instaTouch{
    constructor({ url, download, filepath, filename, filetype, id, count, proxy, queryHash, mediaType, scrapeType, asyncDownload, progress, cli }){
        this._url = url;
            ...
            try{
                ...
                switch(this._filetype){
                    case "json":
                        ...
                        break;
                    case "csv":
                        await Bluebird.fromCallback(cb => fs.writeFile(csv, json2csvParser.parse(this._collector), cb));
                        break;
                    default:
                        await Promise.all([
                            await Bluebird.fromCallback(cb => fs.writeFile(json, JSON.stringify(this._collector), cb)),
