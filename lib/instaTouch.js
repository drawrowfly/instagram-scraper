...
class instaTouch extends EventEmitter{
    constructor({ url, download, filepath, filename, filetype, id, count, proxy, queryHash, mediaType, scrapeType, asyncDownload, progress, cli, event, timeout }){
        super()
        this._url = url;
        ...
        this._id = id; 
        this._toCollect = count || undefined;
        this._proxy = proxy;
    ...
    getPosts(){
        return new Promise( async(resolve, reject) => {
            if (!this._itemId){
                try{
                    if (this._scrapeType === 'likers' || this._scrapeType === 'location' || this._scrapeType === 'followers'){
    ...
    graphqlQuery(ineedhelp){
        return new Promise( async (resolve, reject) => {
            ...
            if (this._scrapeType === "likers"){
                query = JSON.stringify({"shortcode":this._id, "include_reel":true, "first":this._collector.length === 0 ? 24 : 12, "after":this._endCursor});
            }

            if (this._scrapeType === "followers"){
                query = JSON.stringify({"id":this._id, "include_reel":true, "fetch_mutual":false, "first":this._collector.length === 0 ? 24 : 12, "after":this._endCursor});
            }

            if (ineedhelp){
                if (ineedhelp.type === "post"){
                    query = JSON.stringify({"shortcode": ineedhelp.shortcode,"child_comment_count":3,"fetch_comment_count":40,"parent_comment_count":24,"has_threaded_comments":true});
                }
            }
    ...
    _getUsername(id){
        return new Promise( async (resolve, reject) => {
            let options = {
                ...
                headers: {
                    'User-Agent':'Instagram 1.1.1 Android (24/7.0; 420dpi; 1080x1920; samsung; SM-J730FM; j7y17lte; samsungexynos7870; en_US; 139906542)',
            ...
            try{
                ...
                if (response.body.user){
                    return resolve(response.body.user.username)
                }
                resolve('')
            }catch(error){
                resolve('')
    ...
    collectPosts(edges){
        return new Promise( async (resolve, reject) => {
            for(let i = 0; i < edges.length; i++){
                ...
                //     item.owner.username = await this._getUsername(item.owner.id)
                // }
                switch(this._scrapeType){
                    case 'user':
                    case 'hashtag':
